...
function createUser($db, $data) {
    ...
    if ($check->fetch()) {
        sendResponse('A user with that email already exists.', 409);
    }
...
function updateUser($db, $data) {
    ...
    if (isset($data['email']) && $data['email'] !== '') {
        ...
        $dupCheck->execute([':email' => $newEmail, ':id' => $id]);
        if ($dupCheck->fetch()) {
            sendResponse('That email is already in use by another user.', 409);
        }
...
function changePassword($db, $data) {
    ...
    $result = $update->execute([':password' => $newHash, ':id' => $id]);
