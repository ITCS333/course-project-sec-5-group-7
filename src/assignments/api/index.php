<?php
// Suppress PHP error output so warnings never corrupt the JSON response.
// Errors are still written to the server error log via error_log().
ini_set('display_errors', '0');
ini_set('display_startup_errors', '0');
error_reporting(E_ALL);

// Buffer all output so any accidental output is discarded before we send JSON.
ob_start();
...
function updateAssignment(PDO $db, array $data): void
{
    ...
    if (empty($data['id']) || !is_numeric($data['id'])) {
        sendResponse(['success' => false, 'message' => 'A valid id is required.'], 400);
    }
    $id = (int) $data['id'];
...
function sendResponse(array $data, int $statusCode = 200): void
{
    ...
    // TODO: exit;
    ob_end_clean(); // discard any accidental output (PHP warnings, notices, etc.)
