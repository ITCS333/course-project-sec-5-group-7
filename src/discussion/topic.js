...
function renderReplies() {
  ...
  currentReplies.forEach(r => replyListContainer.appendChild(createReplyArticle(r)));
}
...
async function handleReplyListClick(event) {
  ...
  if (!target.classList.contains('delete-reply-btn')) return;
  ...
  try {
    ...
    if (result.success) {
      ...
      renderReplies();
    } else alert(result.message || 'Failed to delete reply');
  } catch (err) {
    console.error(err);
